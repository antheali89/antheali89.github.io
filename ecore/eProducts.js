...
(function () {
  ...
  const eProducts = products.ecore;
  ...
  function init() {
    ...
    let addToCartBtns = document.querySelectorAll(".EC-btn");
  ...
  function populateProducts() {
    for (let i = 0; i < eProducts.length; i++) {
      let product = eProducts[i];

      let productHeader = document.createElement("h2");
      productHeader.innerText = "Product #" + (i + 1);
      productHeader.classList.add("EC-Font", "product-header");
    ...
    document.getElementById("products-container").appendChild(upArrow);
    arrowScroll();
  }
  ...
  function addToCart() {
    let product =
      eProducts[this.parentElement.parentElement.dataset["prodnum"]];
    ...
    if (cartItem != undefined) {
      ...
      productList.setAttribute(
        "data-prodnum",
        this.parentElement.parentElement.dataset["prodnum"]
      );
  ...
  function arrowScroll() {
    ...
    for (let p = 0; p < pLocation.length - 1; p++) {
      arrows[p].addEventListener("click", function () {
        pLocation[p + 1].scrollIntoView();
      });
    }

    let upArrow = document.getElementsByClassName("up");
    upArrow[0].addEventListener("click", function () {
      pLocation[0].scrollIntoView();
    });
  }
})();
